...
The guard above lives inside the worker, so it sees what MLX reports: active memory. It cannot see what macOS charges the process as a whole, which also includes MLX's retained buffer cache, the weights and the interpreter, and nothing outside the worker owns its process group. [mlx-guard](https://github.com/IonDen/mlx-guard) covers that side. It is a small native supervisor that launches the worker, samples its OS-accounted footprint from outside, and stops the whole process group against a limit you set. The benchmark runner can put every condition under it:
...
The wired limit, the memory ceiling watchdog and the wall-time backstop described above stay on inside a supervised worker. The two layers watch different numbers, and either one can stop a run. If both fire at once, the watchdog's breach record wins and the checkpoint stands down.
