"""T9b rung B1 saturation sweep — dQ MMA d_slab ladder at the flagship canary.

T6-rung-2b discipline: judge tile shapes ONLY at occupancy saturation (flagship shape,
tail query rows vs FULL keys under causal — the production-shaped canary), one artifact
per slab written the instant it finishes (resume = skip-if-exists), winner gets a real
full flagship dQ pass (budget-checked via the launcher, catch-refusal-honestly).

Run (controller, after the rung B1 review approves):
  uv run python _artifacts/attention_bwd_rungs/rungB1_dq_mma_sweep.py   (~1-2 min)
"""
...
SLABS = (16, 32, 64, 128)
SCALAR_BASELINE_G = 35.2   # step1_bwd_timing.json
...
dq2_scalar = launch_bwd_dq(q2, k2, v2, do2, l2, da2, scale=SCALE, causal=True)
mx.eval(q2, k2, v2, do2, l2, da2, dq2_scalar)

results = {}
for slab in SLABS:
    out = ART_DIR / f"rungB1_dq_mma_slab{slab}.json"
    if out.exists():
        results[slab] = json.loads(out.read_text())
        print(f"skip (exists): slab{slab} -> {results[slab]['achieved_g_mac_s']} G")
        continue
    kern = _bwd_dq_kernel(D, True, False, "mma", slab)
    # cross-variant consistency at n=256 (vs the parity-proven scalar kernel)
    dq2_mma = launch_bwd_dq(q2, k2, v2, do2, l2, da2, scale=SCALE, causal=True,
                            variant="mma", d_slab=slab)
    xvar = float(mx.abs(dq2_mma.astype(mx.float32)
                        - dq2_scalar.astype(mx.float32)).max())
    # canary: probe 64 tail rows, size to ~0.25s, median of 5
    rows = 64
    mx.eval(_dispatch_bwd_dq_range(kern, q, k, v, d_o, lse, d_arr, scale_in,
                                   r0=N - rows, r1=N, variant="mma"))
    ...
    rec = {
        "rung": f"rungB1_dq_mma_slab{slab}", "d_slab": slab, "git_sha": GIT_SHA,
...
# Winner's real full flagship pass (launcher-split at the MEASURED rate; refusal is honest)
full_out = ART_DIR / "rungB1_dq_mma_full.json"
if not full_out.exists():
    rate = winner["achieved_g_mac_s"] * 1e9
    rec = {"rung": "rungB1_dq_mma_full", "d_slab": winner["d_slab"],
           "git_sha": GIT_SHA, "mlx": mx.__version__,
           "rate_passed_g": winner["achieved_g_mac_s"]}
    try:
        mx.eval(launch_bwd_dq(q, k, v, d_o, lse, d_arr, scale=SCALE, causal=True,
